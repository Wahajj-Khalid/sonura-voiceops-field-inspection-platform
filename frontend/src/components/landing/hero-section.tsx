...
export const HeroSection: React.FC<HeroSectionProps> = ({ onOpenSampleModal }) => {
  return (
    <section className="relative pt-8 pb-12 sm:pt-14 sm:pb-18 md:pt-20 md:pb-24 px-3 sm:px-6 lg:px-8 max-w-7xl mx-auto w-full">
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 xl:gap-14 items-center relative z-10">
        <div className="lg:col-span-7 space-y-4 sm:space-y-6 text-center lg:text-left flex flex-col items-center lg:items-start">
          <div className="inline-flex items-center space-x-2 px-3 py-1 rounded-full border border-violet-500/30 bg-violet-500/10 text-violet-300 text-[10px] sm:text-xs font-mono max-w-full truncate">
            <Sparkles className="w-3 h-3 text-violet-400 shrink-0" />
            <span className="truncate">Spatial Intelligence for Field Operations</span>
          </div>

          <h1 className="text-2xl sm:text-4xl md:text-5xl lg:text-5xl xl:text-6xl font-extrabold text-white tracking-tight leading-tight font-sans text-center lg:text-left">
            Hands-Free Field Audits Powered by Voice AI
          </h1>

          <p className="text-xs sm:text-base md:text-lg text-slate-300 leading-relaxed font-normal max-w-2xl font-sans text-center lg:text-left">
          ...
          <div className="pt-2 pb-2 w-full sm:w-auto flex justify-center lg:justify-start">
          ...
          <div className="pt-4 sm:pt-6 flex flex-col sm:flex-row flex-wrap items-center justify-center lg:justify-start gap-2.5 sm:gap-6 text-xs font-mono text-slate-400 border-t border-white/5 w-full">
